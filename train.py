...
# pretrained_weights = torch.load('/home/sunh/6D_ws/Fpose_rgb/weights/9.效果更好的from_4080/model_best.pth')['model_state_dict']   ### my weight
...
optimizer = optim.Adam(parameters_to_train, weight_decay=0.00001, lr=0.0001)
    ...
    for poses, rgb, depth, xyz_map, mesh_index, gt_poses in train_loader:
        ...
        for iteration_number in range(iteration):
            ...
            for b in range(0, pose_data.rgbAs.shape[0], bs):
                ...
                # loss_feature_ot = OTLoss(x_rgbd.view(batch_size, -1), x_rgb.view(batch_size, -1))
                # loss_feature_l2 = F.mse_loss(x_rgbd, x_rgb)
                loss = loss_rot * 300 + 200 * loss_trans # + loss_feature_ot *0.0000004 #+ loss_feature_l2
                ...
                print('epoch  {}   batch_number:{} iteration_number:{}   loss_all: {}   ROT: {}     TRAN: {} '.format(
                    epoch, batch_number,iteration_number, loss,loss_rot * 300  , 200 * loss_trans))
                ...
                # trans_delta = trans_residual
                # rot_mat_delta = rot_residual *    cfg['rot_normalizer'] #0.3490658503988659
                # rot_mat_delta = so3_exp_map(rot_mat_delta).permute(0, 2, 1)
