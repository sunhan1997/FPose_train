...
class ScorePredictor:
  ...
  @torch.inference_mode()
  def predict_rgb(self, rgb, depth, K, ob_in_cams, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, net = None):
    '''
    @rgb: np array (H,W,3)
    '''
    logging.info(f"ob_in_cams:{ob_in_cams.shape}")
    ...
    pose_data_my = make_crop_data_batch((28*14,28*14), ob_in_cams, mesh, rgb, depth, K, crop_ratio=self.cfg['crop_ratio'], glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, cfg=self.cfg, mesh_diameter=mesh_diameter)
    pose_data = make_crop_data_batch(self.cfg.input_resize, ob_in_cams, mesh, rgb, depth, K, crop_ratio=self.cfg['crop_ratio'], glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, cfg=self.cfg, mesh_diameter=mesh_diameter)

    feature_b = net(pose_data_my.rgbBs[0: 1, :, :, :])[0][0][0]
    score_list = []
    for i in range(0, 252):
      feature_a = net(pose_data_my.rgbAs[i:i+1, :, :, :])[0][0][0]
      score = OTLoss(feature_a, feature_b)
