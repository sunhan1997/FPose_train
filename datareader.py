...
class YcbineoatReader:
  ...
  def get_depth(self,i):
    ...
    # depth = cv2.imread(depth_path,-1) /1e3
    # ### robi
    # depth_path = self.color_files[i].replace('rgb','depth') ##
    # depth_path = depth_path.replace('RGB','DEPTH') ##
    # depth = cv2.imread(depth_path,-1) * 0.0315 /1e3
    ###### my
    # depth_path = self.color_files[i].replace('rgb', 'depth')  ##
    # depth_path = depth_path.replace('color', 'depth')  ##
    # depth = cv2.imread(depth_path, -1) /1e3
    ...
    depth_path = depth_path.replace('color', 'depth')  ##
    depth = cv2.imread(depth_path, -1) / 1e3
    # depth = cv2.imread(depth_path, -1) /4e3 ###for franka
  ...
  def get_color_real(self,color):
    color = cv2.resize(color, (self.W,self.H), interpolation=cv2.INTER_NEAREST)
    return color

  def get_mask_real(self,mask):
    if len(mask.shape)==3:
      for c in range(3):
        if mask[...,c].sum()>0:
          mask = mask[...,c]
